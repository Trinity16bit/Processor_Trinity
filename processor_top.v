...
module processor_top (
    ...
    // ----------------------------------------------------------
    // Data Memory (SRAM) and Memory-Mapped I/O
    // ----------------------------------------------------------
    // Address 0x00FF: 
    //   - Read: Returns {8'b0, in_port}
    //   - Write: Updates out_port
    // Address 0x00FE:
    //   - Read: Returns {15'b0, enter_btn}
    // ----------------------------------------------------------

    wire is_mmio_sw_addr = (alu_result == 16'h00FF);
    wire is_mmio_btn_addr = (alu_result == 16'h00FE);
    wire is_mmio_addr = is_mmio_sw_addr || is_mmio_btn_addr;

    // MMIO Output Register
    always @(posedge clk) begin
        if (rst) out_port <= 16'h0000;
        else if (mem_write && is_mmio_sw_addr) out_port <= rd_data;
    end

    // Actual Data Memory
    wire [15:0] actual_dmem_read_data;
    data_memory #(
        .MEM_DEPTH (256)
    ) u_dmem (
        .clk        (clk),
        .mem_read   (mem_read && !is_mmio_addr),
        .mem_write  (mem_write && !is_mmio_addr),
        .addr       (alu_result),       // ALU computes the effective address
        .write_data (rd_data),          // ST: rd_data = Rs (data register value)
        .read_data  (actual_dmem_read_data)
    );

    // Mux between MMIO reads and actual RAM reads
    assign dmem_read_data = is_mmio_sw_addr  ? {8'h00, in_port} :
                            is_mmio_btn_addr ? {15'h0000, enter_btn} :
                            actual_dmem_read_data;
